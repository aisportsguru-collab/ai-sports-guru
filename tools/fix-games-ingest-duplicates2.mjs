...
try { execSync('git checkout origin/main -- scripts/ingest/games.mjs', { stdio: 'inherit' }); } catch {}

const path = 'scripts/ingest/games.mjs';
let src = fs.readFileSync(path, 'utf8');
if (src.includes('// UPSERT GUARD INSTALLED')) {
...
})();\n// UPSERT GUARD INSTALLED
`;

fs.writeFileSync(path, hook + src);
console.log('Patched scripts/ingest/games.mjs');

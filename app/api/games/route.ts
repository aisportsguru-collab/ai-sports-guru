  ...
  try {
    ...
    if (league !== "nfl") {
      return NextResponse.json(
        { error: "Only league nfl is supported here" },
        { status: 400 }
      );
    }
    if (!fromStr || !toStr) {
      return NextResponse.json(
        { error: "from and to are required in YYYY MM DD" },
        { status: 400 }
      );
    }
    ...
    if (cached) {
      return NextResponse.json({ data: cached, meta: { league, from: fromStr, to: toStr, source: "cache" } });
    }

    const raw = await fetchOddsRawNFL();
    const normalized = raw.map(flattenToGame);
    const filtered = normalized.filter((g) =>
      withinWindowUTC(g.kickoffISO, fromUTC, toUTCExclusive)
    );

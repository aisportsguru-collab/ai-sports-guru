...
const DAYS_AHEAD = 3;
// Map friendly keys -> TheOddsAPI sport keys
...
export async function POST(req: NextRequest) {
  ...
  try {
    const until = new Date(); until.setDate(until.getDate() + DAYS_AHEAD);

    for (const sport of Object.keys(KEY_MAP)) {
      const oddsKey = KEY_MAP[sport];
      const url = `https://api.the-odds-api.com/v4/sports/${oddsKey}/odds?regions=us&markets=h2h,spreads,totals&oddsFormat=american&dateFormat=iso&apiKey=${apiKey}`;
      const resp = await fetch(url, { cache: 'no-store' });
      if (!resp.ok) throw new Error(`TheOddsAPI ${sport} ${resp.status} ${await safeText(resp)}`);
      ...
      for (const g of games) {
        ...
        const season = String(commence.getUTCFullYear());
        const game_date = commence.toISOString().split('T')[0]; // YYYY-MM-DD
        ...
        if (b) {
          ...
          if (h2h?.outcomes) {
            for (const o of h2h.outcomes) {
              if (o.name === upG.home_team) moneyline_home = o.price;
              if (o.name === upG.away_team) moneyline_away = o.price;
            }
          }
          if (sp?.outcomes) {
            for (const o of sp.outcomes) {
              if (o.name === upG.home_team) { spread_home = o.price; spread_line ??= o.point; }
              if (o.name === upG.away_team) { spread_away = o.price; spread_line ??= o.point; }
            }
          }
          if (tot?.outcomes) {
    ...
    await finishJob(job.id, true, { ok: true });

...
export default async function NFLPage() {
  const { count, rows } = await fetchLeaguePredictions("nfl");
  return (
    <div className="min-h-screen bg-[#0B0B0B]">
      <div className="mx-auto max-w-6xl px-4 py-8">
        <header className="mb-6 flex items-end justify-between">
          <div>
            <h1 className="text-white text-2xl md:text-3xl font-bold">NFL Predictions</h1>
        ...
        <main className="grid grid-cols-1 gap-6">
          {rows.length === 0 && (
            <div className="rounded-2xl border border-[#232632] bg-[#121317] p-6 text-[#A6A6A6]">
              No NFL games found for today. Check back later.
            </div>
          )}
          {rows.map((row: any) => (

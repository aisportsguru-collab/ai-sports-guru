    ...
    <div className="bg-white rounded-lg shadow hover:shadow-md transition-shadow p-4 space-y-4">
      ...
      <div className="grid grid-cols-1 gap-2 text-sm">
        <div>
          ...
          {' '}
          <span className="text-xs text-gray-500">({moneylineConfidence}% confidence)</span>
        </div>
        <div>
          <span className="font-medium">Spread: </span>
          {spreadPick} {spreadValue > 0 ? '+' : ''}
          {spreadValue} <span className="text-xs text-gray-500">({spreadConfidence}% confidence)</span>
        </div>
        <div>
          <span className="font-medium">Total: </span>
          {totalPick} <span className="text-xs text-gray-500">({totalConfidence}% confidence)</span>
        </div>
      </div>
    </div>
  );
